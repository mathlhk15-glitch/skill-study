...
하는 일
  ...
  2. 받은 압축 파일을 original/<id>/ 에 보관 (original/ 은 .gitignore 로 GitHub에 올라가지 않음)
...
--update 를 주면 study.json 만 남기고 기존 폴더 내용을 지운 뒤 새 버전을 풀어서,
새 버전에서 사라진 옛 파일이 남지 않게 합니다. 되돌리려면 Git 기록을 이용하세요.
"""
import argparse
import io
import json
...
def load_index():
    ...
    return {"site": {"title": "AI 스킬 교과서", "tagline": "Agent Skills / SKILL.md 학습", "subtitle": ""},
            "categories": [], "skills": []}
...
def spec_warnings(sid, text):
    ...
    if not re.fullmatch(r"[a-z0-9]+(-[a-z0-9]+)*", name) or len(name) > 64:
        out.append(f"name '{name}' 은 소문자·숫자·하이픈만 쓰는 규격에 맞지 않습니다")
    if name != sid:
        out.append(f"name '{name}' 이 폴더 이름 '{sid}' 과 다릅니다")
    if not (1 <= len(desc) <= 1024):
        out.append(f"description 길이가 {len(desc)}자입니다 (규격: 1~1024자)")
    return out
...
def install_from_zip(zf, archive_name, raw_bytes, args, report):
    ...
    for md in [n for n in names if PurePosixPath(n).name == "SKILL.md"]:
        ...
        (keep / archive_name).write_bytes(raw_bytes)

        text = (dest / "SKILL.md").read_text(encoding="utf-8", errors="replace")
        for w in spec_warnings(sid, text):
            report.append(f"[규격 점검] {sid}: {w}")
